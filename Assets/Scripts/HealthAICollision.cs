...
public class HealthAICollision : MonoBehaviour
{
    public HealthAIManager controller;
    ...
    public void Start()
    {
        if(transform.parent.GetComponent<HealthAIManager>() && controller == false)
        {
            controller = transform.parent.GetComponent<HealthAIManager>();
        }
    }
    public void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            int test;
            if (collision.gameObject.tag == "PlayerBullet" && int.TryParse(collision.gameObject.name, out test))
            {
            ...
            if (collision.gameObject.tag == "PlayerMelee")
            {
                ...
                {
                    if (collision.gameObject.GetComponent<Flame>().enchant && collision.gameObject.GetComponent<Flame>().enchant.enchants[4])
                    {
